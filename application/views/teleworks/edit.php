<?php
/**
 * This view allows an employees (or HR admin) to modify a telework request
...
<h2><?php echo lang('teleworks_edit_title');?><?php echo $telework['id']; ?> <?php echo $help;?>&nbsp;<span class="muted">(<?php echo $name ?>)</span></h2>
<?php
$attributes = array('id' => 'frmTeleworkForm');
if (isset($_GET['source'])) {
  echo form_open('teleworks/edit/' . $id . '?source=' . $_GET['source'], $attributes);
} else {
  echo form_open('teleworks/edit/' . $id, $attributes);
} ?>
<div class="row">
  <div class="span6">
<div class="row-fluid">
...
<?php echo validation_errors(); ?>
    ...
    <label for="viz_startdate"><?php echo lang('teleworks_edit_field_start');?></label>
    <input type="text" name="viz_startdate" id="viz_startdate" value="<?php $date = new DateTime($telework['startdate']); echo $date->format(lang('global_date_format'));?>" autocomplete="off" />
    <input type="hidden" name="startdate" id="startdate" value="<?php echo $telework['startdate'];?>" />
    <select name="startdatetype" id="startdatetype">
        <option value="Morning" <?php if ($telework['startdatetype'] == "Morning") {echo "selected";}?>><?php echo lang('Morning');?></option>
        <option value="Afternoon" <?php if ($telework['startdatetype'] == "Afternoon") {echo "selected";}?>><?php echo lang('Afternoon');?></option>
    </select><br />

    <label for="viz_enddate"><?php echo lang('teleworks_edit_field_end');?></label>
    <input type="text" name="viz_enddate" id="viz_enddate" value="<?php $date = new DateTime($telework['enddate']); echo $date->format(lang('global_date_format'));?>" autocomplete="off" />
    <input type="hidden" name="enddate" id="enddate" value="<?php echo $telework['enddate'];?>" />
    <select name="enddatetype" id="enddatetype">
        <option value="Morning" <?php if ($telework['enddatetype'] == "Morning") {echo "selected";}?>><?php echo lang('Morning');?></option>
        <option value="Afternoon" <?php if ($telework['enddatetype'] == "Afternoon") {echo "selected";}?>><?php echo lang('Afternoon');?></option>
    </select><br />
    <label for="duration"><?php echo lang('teleworks_edit_field_duration');?> <span id="tooltipDayOff"></span></label>

    <?php if ($this->config->item('disable_edit_telework_duration') == TRUE) { ?>
    <input type="text" name="duration" id="duration" value="<?php echo $telework['duration']; ?>" readonly />
    <?php } else { ?>
    <input type="text" name="duration" id="duration" value="<?php echo $telework['duration']; ?>" />
    <?php } ?>
    <input type="hidden" name="type" id="type" value="<?php echo $telework['type'];?>" />
    ...
    <label for="cause"><?php echo lang('teleworks_edit_field_cause');?></label>
    <textarea name="cause"><?php echo $telework['cause']; ?></textarea>
    <br/>
    ...
    <?php if ($is_hr) {?>
    <label for="status"><?php echo lang('teleworks_edit_field_status');?></label>
    <select name="status" class="<?php echo $style; ?>">
        <option value="1" <?php if ($telework['status'] == LMS_PLANNED) echo 'selected'; ?>><?php echo lang('Planned');?></option>
        <option value="2" <?php if (($telework['status'] == LMS_REQUESTED) || $this->config->item('telework_status_requested')) echo 'selected'; ?>><?php echo lang('Requested');?></option>
        <option value="3" <?php if ($telework['status'] == LMS_ACCEPTED) echo 'selected'; ?>><?php echo lang('Accepted');?></option>
        <option value="4" <?php if ($telework['status'] == LMS_REJECTED) echo 'selected'; ?>><?php echo lang('Rejected');?></option>
        <option value="5" <?php if ($telework['status'] == LMS_CANCELLATION) echo 'selected'; ?>><?php echo lang('Cancellation');?></option>
        <option value="6" <?php if ($telework['status'] == LMS_CANCELED) echo 'selected'; ?>><?php echo lang('Canceled');?></option>
    </select>
    <?php } else { ?>
    <label for="status"><?php echo lang('teleworks_edit_field_status');?></label>
    <select name="status" class="<?php echo $style; ?>">
    ...
    <?php if ($is_hr) {?>
    <button type="submit" class="btn btn-primary"><i class="mdi mdi-check"></i>&nbsp;<?php echo lang('teleworks_edit_button_update');?></button>&nbsp;
    <?php } ?>

    <?php if (isset($_GET['source'])) {?>
        <a href="<?php echo base_url() . $_GET['source']; ?>" class="btn btn-danger"><i class="mdi mdi-close"></i>&nbsp;<?php echo lang('teleworks_edit_button_cancel');?></a>
    <?php } else {?>
        <a href="<?php echo base_url(); ?>teleworks" class="btn btn-danger"><i class="mdi mdi-close"></i>&nbsp;<?php echo lang('teleworks_edit_button_cancel');?></a>
    <?php } ?>
...
<script type="text/javascript">
    ...
    var userId = <?php echo $telework['employee']; ?>;
    var teleworkId = <?php echo $telework['id']; ?>;
...
function validate_form() {
    ...
    if ($('#viz_startdate').val() == "") fieldname = "<?php echo lang('teleworks_edit_field_start');?>";
    if ($('#viz_enddate').val() == "") fieldname = "<?php echo lang('teleworks_edit_field_end');?>";
    if ($('#duration').val() == "" || $('#duration').val() == 0) fieldname = "<?php echo lang('teleworks_edit_field_duration');?>";
    if (fieldname == "") {
        return true;
    } else {
...
$(function () {
    //Selectize the telework type combo
    //$('#type').select2();

    //On opening, refresh telework request information
    refreshTeleworkInfo();
